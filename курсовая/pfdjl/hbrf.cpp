...
public:
	...
	void shag1() {
		...
		else if (!(n >= 'a' && n <= 'z') && !(n >= 'A' && n <= 'Z') && !(n == '1')) {
			cout << "Unknown state. Exiting." << endl;
			exit(0);
		}
	...
	void shag2a() {
		...
		cout << "end of the cycle. " << endl << endl;
	...
	void shag6() {
		cout << "The end of the conveyor. " << endl << endl;
		sens.seti0(1);
		cnvr.setq2(1);
		shag7();
	}
...
int main() {
	...
	int op = 0; 
	while (op != 9) {
		cout << "Enter an option 1(to start programm) or 2 (to exit): ";
		cin >> op;
		switch (op) {
		case 1: controller.shag1();
			break;
		case 2: cout << "Exiting..." << endl;
			break;
		default: cout << "Unknown state. Try again." << endl;
			break;
		}
	}
